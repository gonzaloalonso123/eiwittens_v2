...
const ErrorDisplay = ({ error }) => (
  <h1 className="bg-red-500/10 border-2 p-2 rounded-md border-red-500 text-red-500 flex items-center gap-2">
    <MdError /> {error}
  </h1>
);
...
const TestScraper = ({ actions, url, setError }) => {
  ...
  const test = async () => {
    ...
    if (error.index != -1) {
      setError({ index: error.index, text: error.text });
    }
    setLoading(false);
  };
    ...
    <div className="flex flex-col gap-6">
      {value?.map((c, i) => (
        <OneAction
          ...
          error={error?.index === i ? error.text : null}
        />
      ))}
