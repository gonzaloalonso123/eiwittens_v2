...
export const QuickFixer = () => {
	...
	const save = async () => {
		if (storedActions[currentIndex].length > 0) {
			console.log('saving...')
			await updateProduct(products[currentIndex].id, {
				scraper: storedActions[currentIndex],
				warning: false,
			});
			toast.success("Product fixed");
...
const NavigateButtons = ({ currentIndex, setCurrentIndex, total, save }) => {
	...
	return (
		<div className="flex gap-4 py-4 justify-between">
			...
			>
				{currentIndex === total - 1 ? "Save And Finish" : "Save And Next"}
			</Button>
		</div>
	);
};
