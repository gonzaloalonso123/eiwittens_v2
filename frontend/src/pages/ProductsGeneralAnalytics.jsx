...
const prepareByProduct = (products, range) => {
	...
	while (currentDate <= end) {
		...
		products.forEach(product => {
			const clicks = product.count_clicked.filter(click => click.split('T')[0] === currentDate.toISOString().split('T')[0]).length;
...
const prepareByAllProducts = (products, range) => {
	...
	while (currentDate <= end) {
		const clicks = products.reduce((acc, p) => {
			return acc + p.count_clicked?.filter(click => click.split('T')[0] === currentDate.toISOString().split('T')[0]).length;
		}, 0);
		data.push({ name: currentDate.toISOString().split('T')[0], clicks });
		currentDate.setDate(currentDate.getDate() + 1);
	}
	return data;
}
...
export const ProductsGeneralAnaltyics = () => {
	...
	const [dateRange, setDateRange] = useState({ start: new Date("2024-10-24"), end: new Date() });
	...
	useEffect(() => {
		if (products) {
			const productsWithClicks = products.filter(p => p.count_clicked?.length > 0);
			const d = infoType === "total" ? prepareByAllProducts(productsWithClicks, dateRange) : prepareByProduct(productsWithClicks, dateRange);
			setData(d)
			setLegend(
				infoType === "total" ?
					[{ name: "Clicks", clicks: d.reduce((acc, e) => acc + e.clicks, 0), key: "clicks" }]
					: products
						.filter(p => p.count_clicked?.length > 0)
						.sort((a, b) => b.count_clicked.length - a.count_clicked.length)
...
const InfoSelector = ({ setter, selectedOption }) => {
	return (
		<div className="flex border divide-x rounded-md w-fit">
			<InfoSelectorOption icon={<GrGraphQl />} text="Total clicks" setter={() => setter("total")} selected={selectedOption == "total"} />
			<InfoSelectorOption icon={<MdAutoGraph />} text="By Product" setter={() => setter("by_product")} selected={selectedOption == "by_product"} />
		</div>
	)
}
...
const InfoSelectorOption = ({ icon, text, setter, selected }) => {
	return (
		<div className={`flex gap-2 w-fit items-center p-2 px-4 cursor-pointer ${selected ? 'bg-gray-100' : 'bg-white'}`} onClick={setter}>
			{icon}
			<label>{text}</label>
		</div>
	)
	...
	return (
		<div className="mt-4 w-64">
			<RangePicker
				...
				onChange={(date, dateString) => setter({ start: new Date(date[0]), end: new Date(date[1]) })}
